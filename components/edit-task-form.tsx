...
import { Select, SelectItem, SelectContent, SelectTrigger, SelectValue, SelectGroup } from "./ui/select";
import { Loader } from "lucide-react";
import { Button } from "./ui/button";
import { EditTaskFormValues, editTaskSchema } from "@/schema/task";
import { useMutation } from "@tanstack/react-query";
import { Card, CardContent, CardHeader, CardTitle } from "./ui/card";  // Import the Card components
import { createTask, editTask } from "@/http/tasks";
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormDescription>Provide a brief description of the task.</FormDescription>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            />
                                        ...
                                        <FormItem>
                                            ...
                                            <FormControl>
                                                <Input
                                                    type="date"
                                                    {...field}
                                                    aria-invalid={form.formState.errors.due_date ? "true" : "false"}
                                                />
                                            </FormControl>
                                            <FormDescription>Choose the due date for this task.</FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            ...
                                            <FormDescription>Select the task's current status.</FormDescription>
                                            <FormMessage />
                                        </FormItem>
                                    )}
                                />
                            ...
                            <Button type="submit" disabled={isPending} className="w-full cursor-pointer">
                                {isPending ? <Loader className="h-4 w-4 animate-spin" /> : "Edit Task"}
                            </Button>
                        </form>
                    </Form>
                </CardContent>
